...
class branches{
    ...
    constructor(nome: string, dataCriação: Date){
       this.nome = nome
       this.dataCriacao = dataCriação
     }

    criarCommit(): void {
        let codigo = rl.question('Digite o codigo: ')
        return codigo
    }
    
    public merge(outraBranch: branches){
        let inserir = this.commits.concat(outraBranch.commits)

    return inserir 

    }
    checkout(): void{
            let escolha + rl. question('qual branch vc quer escolher')
        }
